...
initial()->
 ...
 %   [io:format("~p~n",[{Node,rpc:call(Node,mnesia,system_info,[],2*1000)}])||Node<-get_nodes()],
    ...
    %%-- db_host test
    [{atomic,ok},{atomic,ok},{atomic,ok}]=rpc:call(Node0,dbase_infra,load_from_file,[db_host,?HostConfiguration],5*1000),    
    [{host0@c100,host1@c100},
     {host1@c100,{badrpc,_}},
     {host2@c100,{badrpc,_}}]=[{Node,rpc:call(Node,db_host,node,[{"c100","host1"}],5*1000)}||Node<-get_nodes()],

    %%-- db_service_catalog test
    rpc:call(Node0,dbase_infra,load_from_file,[db_service_catalog,?ServiceCatalog],5*1000),    
    [{host0@c100,"https://github.com/joq62/myadd.git"},
     {host1@c100,{badrpc,_}},
     {host2@c100,{badrpc,_}}]=[{Node,rpc:call(Node,db_service_catalog,git_path,[{myadd,"1.0.0"}],5*1000)}||Node<-get_nodes()],
    
    %%-- db_deployment test
    rpc:call(Node0,dbase_infra,load_from_file,[db_deployment,?Deployments],5*1000),    
    [{host0@c100,[{myamath,"1.0.0",1,["C100"]},
		  {myadd,"1.0.0",2,[]},
		  {mydivi,"1.0.0",2,[]}]},
     {host1@c100,{badrpc,_}},
     {host2@c100,{badrpc,_}}
    ]=[{Node,rpc:call(Node,db_deployment,info,[dep1],5*1000)}||Node<-get_nodes()],
    
    [{host0@c100,[{myadd,"1.0.0",1,["c100"]}]},
     {host1@c100,{badrpc,_}},
     {host2@c100, {badrpc,_}}
    ]=[{Node,rpc:call(Node,db_deployment,info,[dep2],5*1000)}||Node<-get_nodes()],
...
add_node()->
    ...
    ok=rpc:call(Node1,dbase,dynamic_load_table,[db_service_catalog],3*1000),
    ok=rpc:call(Node1,dbase,dynamic_load_table,[db_deployment],3*1000),
    timer:sleep(500),
    ...
    ok=rpc:call(Node2,dbase,dynamic_load_table,[db_service_catalog],3*1000),
    ok=rpc:call(Node2,dbase,dynamic_load_table,[db_deployment],3*1000),
    timer:sleep(500),
    
    host1@c100=rpc:call(Node1,db_host,node,[{"c100","host1"}]),
    host2@c100=rpc:call(Node2,db_host,node,[{"c100","host2"}]),
    "https://github.com/joq62/myadd.git"=rpc:call(Node1,db_service_catalog,git_path,[{myadd,"1.0.0"}]),
    "https://github.com/joq62/myadd.git"=rpc:call(Node2,db_service_catalog,git_path,[{myadd,"1.0.0"}]),
    not_deployed=rpc:call(Node1,db_deployment,status,[dep2]),
    not_deployed=rpc:call(Node2,db_deployment,status,[dep2]),
    ...
    ok=rpc:call(Node0,dbase,dynamic_load_table,[db_service_catalog],3*1000),
    ok=rpc:call(Node0,dbase,dynamic_load_table,[db_deployment],3*1000),
    timer:sleep(500),

    host1@c100=rpc:call(Node0,db_host,node,[{"c100","host1"}]),
    host1@c100=rpc:call(Node1,db_host,node,[{"c100","host1"}]),
    host2@c100=rpc:call(Node2,db_host,node,[{"c100","host2"}]),
    not_deployed=rpc:call(Node0,db_deployment,status,[dep2]),
    not_deployed=rpc:call(Node1,db_deployment,status,[dep2]),
    not_deployed=rpc:call(Node2,db_deployment,status,[dep2]),
